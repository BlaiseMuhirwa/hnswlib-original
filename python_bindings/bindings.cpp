...
public:
  explicit CustomFilterFunctor(
      const std::function<bool(hnswlib::labeltype)> &f) {
    filter = f;
  }
...
get_input_ids_and_check_shapes(const py::object &ids_, size_t feature_rows) {
...
public:
  ...
  size_t indexFileSize() const { return appr_alg->indexFileSize(); }
  ...
  void loadIndex(const std::string &path_to_index, size_t max_elements,
    ...
    appr_alg = new hnswlib::HierarchicalNSW<dist_t>(
        l2space, path_to_index, false, max_elements, allow_replace_deleted);
  ...
  py::object getData(py::object ids_ = py::none(),
                     std::string return_type = "numpy") {
    std::vector<std::string> return_types{"numpy", "list"};
    if (std::find(std::begin(return_types), std::end(return_types),
                  return_type) == std::end(return_types)) {
      throw std::invalid_argument(
          "return_type should be \"numpy\" or \"list\"");
    }
    std::vector<size_t> ids;
    if (!ids_.is_none()) {
      ...
      if (ids_numpy.ndim == 0) {
        throw std::invalid_argument("get_items accepts a list of indices and "
    ...
    if (return_type == "numpy") {
      return py::array_t<data_t, py::array::c_style | py::array::forcecast>(
          py::cast(data));
    }
  }
  ...
  py::dict getAnnData() const { /* WARNING: Index::getAnnData is not thread-safe
    ...
    return py::dict(
        ...
        "label_lookup_internal"_a = py::array_t<hnswlib::tableint>(
            {appr_alg->label_lookup_.size()}, // shape
            {sizeof(hnswlib::tableint)}, // C-style contiguous strides for each
                                         // index
  ...
  void unmarkDeleted(size_t label) { appr_alg->unmarkDelete(label); }
...
template <typename dist_t, typename data_t = float> class BFIndex {
public:
  static const int ser_version = 1; // serialization version
  ...
  size_t getMaxElements() const { return alg->maxelements_; }

  size_t getCurrentCount() const { return alg->cur_element_count; }
